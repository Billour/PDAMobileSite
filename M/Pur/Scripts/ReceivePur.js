...
$(document).ready(function () {
    ...
    $('#txtRecQty').bind('change', function () {
        ...
        if (isNaN(num)) {
            ...
            if (parseInt(num, 10) < 0) {
                ...
                //取小數點1位
                $('#txtRecQty').val(new Number(num).toFixed(1));
                if (!checkqty())////允收數量檢查
                {
                    $('#txtRecQty').val(oldData);
                    oldData = null;
    ...
    $('#liPur').bind('click', function () {
        if ($('#lblVendorName').text() == "")
            ...
            $('#tablePur').html('');
            ClearOrderSort();//20160801清空排序

            //清空查詢條件
            $('#txtrec_pur_nbr').val('');
            $('#txtrec_item_no').val('');
            $("#rblrecsorta").prop("checked", true);
            $('input[name=rblrecsort]').checkboxradio("refresh");
            $.ajax({
                async: true,
                type: 'POST',
                url: sitepath + '/M/Pur/ashx/ReceivePur.ashx',
                data: {
                    FunType: 'QueryPur3Day',
                    TableID: 'tbPur',
                    PlanRecvDate: $('#txtPlanRecvDate').val(),
                    VendorID: $('#txtVendor').val(),
                    KeyList: getKeyList(),
                    checkAll: 'check3Dayall(\"{0}\");',
                    checkeach: 'check3Day(\"{0}\");'
                },
                dataType: 'html',
                timeout: (30000),
                success: function (data) {
                    if (!!data) {
                        $('#tablePur').html(data);
                        $("[id^=tbPur_checkRec_]").each(function () {
                            if ($(this).text() == "false") {
                                $(this).text('否');
                                $(this).css('color', 'red').css('font-weight', '900');
                            } else {
                                $(this).text('是');
                            }
                        });
    ...
    //#region 搜尋採購單 20160715 有過濾條件  event
    //20160531修改成三日前不可收料
    $('#btnrec_nbrQuery').bind('click', function () {
        ClearOrderSort();//20160801清空訂單排序
        if ($('#lblVendorName').text() == "")
            alert("請先輸入供應商");
        else {
            $.mobile.changePage('#diaPur');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    checkAll: 'check3Dayall(\"{0}\");',
                    checkeach: 'check3Day(\"{0}\");',
                    pur_nbr: $('#txtrec_pur_nbr').val(),
                    item_no: $('#txtrec_item_no').val(),
                    data_sort: $('input[name=rblrecsort]:checked').val()
                },
                dataType: 'html',
                timeout: (30000),
                success: function (data) {
    ...
    //#region 搜尋採購單確定 event 20160802增加自訂排序
    $('#btnPurOk').bind('click', function () {
        //alert(JSON.stringify(getordersrt)); //20160801  test
            ...
            if (selectPur != '') {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        TableID: 'tbRec',
                        PurIDs: selectPur,
                        CheckBoxFun: 'checkBoxPur(\"{0}\")',
                        ButtonFun: 'buttonPur(\"{0}\")',
                        EndIndex: getSeq(),
                        Sort: JSON.stringify(getordersrt)
                    }
                    ,
                    dataType: 'json',
                    timeout: (30000),
                    success: function (data) {
                        if (!!data) {
                            ...
                            //20160715 料號藍色顯示
                            $('[id^=tbRec_item_no]').css("color", "fuchsia").css("font-weight", "bold");
                            //=======html處理=======
                            ...
                            //=======json處理=======
                            ClearOrderSort();//20160801 清空排序資料
                            hideDelete();
                        }
                    },
                    error: function (xhr, ajaxOptions, thrownError) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.length > 0) {
                    ...
                    $('#txtRecQty').val(Number(data[0].rec_qty).toFixed(1));
            ...
            try {
                ...
                if (beforeOK) {
                    $.ajax({
                        ...
                        data: { FunType: 'CallERP', OrderType: 'RV11', OrderNo: orderNo },
                        dataType: 'json',
                        // timeout: (60000),
                        success: function (data) {
                            //判斷轉檔是否成功
                            if (data[0].Result.trim() == 'True') {
                                ...
                                $('#head').text(head);//台頭
                                //show出訊息，詢問列印
                                if (confirm(data[0].Message.trim() + "\n 請問你是否要列印" + orderNo + "收料單?")) {
                                    //#region 列印單據
                                    $.ajax({
                                        async: false,
                                        type: 'POST',
                                        url: sitepath + '/M/ashx/Common.ashx',
                                        data: { FunType: 'CallERPPrint', PrintNo: $("[id$=ddlprint]").val(), OrderType: 'RV11', OrderNo: data[0].ERP.trim() },
                                        dataType: 'text',
                                        timeout: (180000),
                                        success: function (data) {
...
//#region 20160715 檢查三天前不可收料
//全選
function check3Dayall(id) {

    //全選打抅
    var Allcheckbox = "[id^=" + id + "_cb_]";
    var checkboxID = "#" + id + "_all";
    var msg = "";
    $(Allcheckbox).prop("checked", $(checkboxID).is(":checked"));
    $('input' + Allcheckbox + ':checked').each(function () {
        msg += check3DaybyData($(this).attr('id').replace('tbPur_cb_', ''))
    });
    if (msg.length > 0) alert(msg);
    ClearOrderSort();
}
//單筆
function check3Day(id) {
    var msg = check3DaybyData(id);
    if (msg.length > 0)
        alert(msg);

    if ($("#tbPur_cb_" + id).prop("checked"))
        AddOrderSort(id);
    else
        DelOrderSort(id);

    //alert(JSON.stringify(getordersrt));
}

function check3DaybyData(id) {
    var result = "";
    if ($("#tbPur_checkRec_" + id).text() == "否") {
        $("#tbPur_cb_" + id).prop("checked", false);
        result = "\n【採購單" + id + "，採購的預計到廠日" + $("#tbPur_plan_recv_date_" + id).text() + "大於收料日期3天以上，不能存檔! (如需存檔，請與採購部人員聯絡，變更採購明細之預計到廠日)】\n";
    }
...
function buttonPur(id) {
    ...
    for (var i = 0; i < gJson.length; i++) {
        if ((gJson[i].pur_nbr + gJson[i].pur_seq) == id) {
            ...
            $('#txtRecQty').val(new Number(gJson[i].rec_qty).toFixed(1));
            $('#txtPurQty').val(new Number(gJson[i].pur_qty).toFixed());
            ...
            $.ajax({
                ...
                type: 'POST',
                url: sitepath + '/M/Pur/ashx/ReceivePur.ashx',
                data: { FunType: 'getplan_recv_date', ID: gJson[i].pur_nbr + gJson[i].pur_seq },
                dataType: 'text',
                timeout: (30000),
                success: function (data) {
                    if (!!data) {
                        $('#txtplan_recv_date').val(data);
                    }
                },
                error: function (xhr, ajaxOptions, thrownError) {
                    alertException(xhr, ajaxOptions, thrownError);
...
//#region  取得資料明細
function GetDetail(rec_nbr, keyin_date) {
    $('#lisOrder').empty();
    $.ajax({
        async: true,
        type: 'POST',
        url: sitepath + '/M/Pur/ashx/ReceivePur.ashx',
        data: { FunType: 'getOrderDetailRV11', rec_nbr: rec_nbr },
        dataType: 'json',
        timeout: (30000),
        success: function (data) {
            if (!!data) {
...
//#region 20160801 清空訂單排序
function ClearOrderSort() {
    getordersrt = null;
}
//#endregion 

//#region 20160801 新增訂單排序
function AddOrderSort(Key) {
    var ordersort = {};
    ordersort.Key = Key;

    if (getordersrt == null)
        getordersrt = [ordersort];
    else
        getordersrt.push(ordersort);
}
//#endregion 

//#region 20160801 刪除訂單排序
function DelOrderSort(Key) {
    if (getordersrt != null)
        for (var i = getordersrt.length - 1; i >= 0; i--) {
            if (getordersrt[i].Key == Key) {
                getordersrt.splice(i, 1);  //刪除                            
                break;
            }
        }
}
