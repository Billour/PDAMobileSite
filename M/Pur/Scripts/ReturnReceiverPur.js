...
$(document).ready(function () {
    ...
    $('#txtreturn_qty').bind('change', function () {
        ...
        if (isNaN(num)) {
            ...
            if (parseInt(num, 10) < 0) {
                ...
                //取小數點1位
                $('#txtreturn_qty').val(new Number(num).toFixed(1));
            }
        }
    ...
    //#region 搜尋採購單 20160803 有過濾條件  event    
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.length > 0) {
                    ...
                    $('#txtreturn_qty').val(new Number(data[0].return_qty).toFixed(1));
...
//#region 20160715 搜尋收料單checkbox檢查
//全選
function CheckAll(id) {
    ...
    $('input' + Allcheckbox + ':checked').each(function () {
        checkOne($(this).attr('id').replace('tbRec_cb_', ''))
    });
